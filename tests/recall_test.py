...
def test_get_recall():
    ...
    got_vcf_all, got_vcf_filtered = recall.get_recall(
        ref_fasta, vcf_to_test, tmp_out, 500, debug=True, truth_fasta=truth_fasta,
    )
    ...
    mask["truth"].add(180)
    got_vcf_all, got_vcf_filtered = recall.get_recall(
        ref_fasta, vcf_to_test, tmp_out, 500, debug=True, truth_fasta=truth_fasta, truth_mask=mask,
    )
